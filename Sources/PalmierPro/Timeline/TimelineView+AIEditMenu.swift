...
extension TimelineView {
    func aiEditSubmenu(for clipId: String) -> NSMenu? {
        ...
        let addAction: (EditAction) -> Void = { action in
            ...
            switch action {
            case .upscale:
                let upscaleItem = NSMenuItem(title: paidBlocked ? loc("Upscale… (Paid)") : loc("Upscale…"), action: #selector(self.performAIEditUpscale(_:)), keyEquivalent: "")
            ...
            case .reframe:
                let item = NSMenuItem(title: paidBlocked ? loc("Reframe… (Paid)") : loc("Reframe…"), action: #selector(self.performAIEditReframe(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = clipId
                item.isEnabled = aiAllowed && !paidBlocked
                submenu.addItem(item)
            case .lipSync:
                let item = NSMenuItem(title: paidBlocked ? loc("Lip Sync… (Paid)") : loc("Lip Sync…"), action: #selector(self.performAIEditLipSync(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = clipId
                item.isEnabled = aiAllowed && !paidBlocked
                submenu.addItem(item)
            case .edit:
                let item = NSMenuItem(title: paidBlocked ? loc("Edit… (Paid)") : loc("Edit…"), action: #selector(self.performAIEditEdit(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = clipId
                item.isEnabled = aiAllowed && !paidBlocked
                submenu.addItem(item)
            case .generateMusic, .generateSFX:
                ...
                let item = NSMenuItem(title: "\(loc(kind.title))…", action: #selector(self.performAIEditVideoAudio(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = ["clipId": clipId, "kind": action == .generateMusic ? "music" : "sfx"]
                item.isEnabled = aiAllowed
                submenu.addItem(item)
            case .rerun:
                let item = NSMenuItem(title: loc("Rerun"), action: #selector(self.performAIEditRerun(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = clipId
                item.isEnabled = aiAllowed
                submenu.addItem(item)
            case .createVideo:
                let createItem = NSMenuItem(title: loc("Create Video"), action: nil, keyEquivalent: "")
                let createMenu = NSMenu()
                createMenu.autoenablesItems = false
                let mk: (String, Bool) -> NSMenuItem = { title, asReference in
                    let item = NSMenuItem(title: loc(title), action: #selector(self.performAIEditCreateVideo(_:)), keyEquivalent: "")
                    item.target = self
                    item.representedObject = ["clipId": clipId, "asReference": asReference]
                    item.isEnabled = aiAllowed
                    return item
                }
                createMenu.addItem(mk(loc("Set as first frame"), false))
                createMenu.addItem(mk(loc("Set as reference"), true))
        ...
        if !enhanceActions.isEmpty {
            submenu.addItem(.sectionHeader(title: loc("AI Enhance")))
